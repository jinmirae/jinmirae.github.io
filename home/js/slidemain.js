...
function play_w(directw) { //좌우로 슬라이드되는 함수선언(왼쪽|오른쪽)
	...
	if(directw=='right') {
		slideNum = slideNum + 1; //슬라이트 번호 증가
	...
	//li 태그에서 클래스 영역안쪽의 li 태그에서 클래스 seq인것 3개를 each함수로 반복(3번반복)
	...
	//플래그(귓발) true(자동슬라이드), false(슬라이드멈춤)
	if(slideAuto){//ture 일때
