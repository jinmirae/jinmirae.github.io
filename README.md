...
- html5내장된 유효성검사? required(공백체크), type="email"(이메일형식체크), type="password"(입력값을 숨김처리), type="digits"(숫자체크)
...
- 기획시, 메인1 CSS 네이밍, 서브1 CSS 네이밍.(과장/이사급이 2개 파일 만듬) -> 기반으로 퍼블리셔, 프론트개발자 CSS, HTML 작업을 이어서 합니다.
